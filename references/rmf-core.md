...
> **ID scheme warning:** This narrative file uses shortened labels (`GV-1.1`, `MP-…`, `MS-…`, `MG-…`).  
> For **Assessment Mode scoring and report findings**, always use Playbook IDs from `playbook.json` / `playbook-packs/` (`GOVERN 1.1`, `MAP 1.1`, …).  
> Do not mix schemes in assessment reports. Mapping is approximate — Playbook text wins on conflicts.
